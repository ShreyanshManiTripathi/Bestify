...
export class BoardComponent implements OnInit {
  ...
  resetGame() {
    ...
    this.time = { start: 0, elapsed: 0, level: 800  };
    this.paused = false;
    this.addOutlines();
  }
  ...
  gameOver() {
    ...
    cancelAnimationFrame(this.requestId);
    this.service.getGameTransaction(this.points,this.status,this.userId,this.games[0].gameId).subscribe(response =>{
      console.log(response);
    });
    this.highScore = this.points > this.highScore ? this.points : this.highScore;
    localStorage.setItem(this.ngxTetris, JSON.stringify({ 'best_score':this.highScore }));
    this.ctx.fillStyle = 'black';
